...
function buildTodoListItem(todo) {
  ...
  // Get the first child of the list and insert the new todo before it
  const firstChild = todoList.firstChild;
  todoList.insertBefore(todoItem, firstChild);
}
...
function addTodo(e) {
  ...
  const todoText = todoInput.value.trim(); // trim any whitespace
  if (!todoText) {
